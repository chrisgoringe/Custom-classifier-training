...
def main():
    ...
    if Args.model.endswith(".safetensors"):
        predictor = AestheticPredictor.from_pretrained(pretrained=Args.model, image_directory=Args.top_level_image_directory)
        predictor.eval()

    with torch.no_grad():
        df = DataFrame()
        database_scores = ImageScores.from_scorefile(top_level_directory=Args.top_level_image_directory, scorefilename=Args.scorefile)
        if predictor:
            predictor.precache(database_scores.image_files(fullpath=True))
            model_scores = ImageScores.from_evaluator(predictor.evaluate_file, database_scores.image_files(), Args.top_level_image_directory)
        elif Args.model.endswith(".json"):
            model_scores = ImageScores.from_scorefile(Args.top_level_image_directory, Args.model)
        else:
            print("Args.model should be a model.safetensors file or a score.json file")
            return
        splits = {}
        if Args.splitfile:
            split_path = os.path.join(Args.top_level_image_directory, Args.splitfile)
            if os.path.exists(split_path):
                splits = json.load(open(split_path))

        df['image'] = database_scores.image_files(fullpath=True)
        df['split'] = list(splits.get(os.path.relpath(f,Args.top_level_image_directory),"") for f in df['image'])
